...
Feature: Playwright Merchant Tests
...
@PRTest
Scenario: Merchant List and Navigation

	Given I click on the My Merchants sidebar option
	Then I am presented with the Merchants List Screen
	# Flexible validation - checks that merchants are displayed but allows for backend changes
	And the merchant list contains at least 3 merchants
	And the merchant list displays merchant names
	And the merchant list displays settlement schedules
	And the merchant list displays contact information
...
Scenario: View Merchant Details
	...
	And the merchant details section is visible
	And the merchant operators section is visible
	And the merchant contracts section is visible
	And the merchant devices section is visible
